...
To add a new provider (e.g. `openai`, `tesseract`) to the system, implement needed adapters (`OCR` + `TextStructuring` or `VisualStructuring`) and register them in the corresponding provider registries.
...
You can have different providers for `OCR` and `TextStructuring` within a document understanding pipeline (e.g. Tesseract and OpenAI respectively). OCR + TextUnderstanding is a two-step pipeline and can involve multiple technologies.
